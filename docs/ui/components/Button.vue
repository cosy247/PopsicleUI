...
const className = computed(() => {
  ...
  if (props.shape) classList.push(props.shape);
  if (props.type) classList.push(props.type);
  if (props.disable) classList.push('disable');
  if (props.loading) classList.push('loading disable');
  return classList.join(' ');
});
const style = computed(() => {
  let style: { fontSize?: string } = {};
  if (props.size) {
